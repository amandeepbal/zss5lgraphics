sap.ui.define([
	...
	return function (client, data, myParent, x, y) {
		...
		this.render = function () {
			...
			if (data.model["INCLUDE.FAN.BEAM"] != undefined) {
				fb = data.model["INCLUDE.FAN.BEAM"].value;
				if (fb === undefined) {
					fb = false;
				}
			}
			...
			case "3":
				...
				if (data.model["IS.HYBRID.PANEL"] != undefined && data.model["IS.HYBRID.PANEL"].value === true) {
					color = data.model["HYBRID.COLOR"].value;
					ishybrid = true;
				};
				break;
			case "5":
			...
			if (rs === '3' && !data.isGable && !ishybrid) {
				var aw = this.client.convert(1);
				var maxx = parseInt((this.x2 - this.x1) / aw) - 1;
				var ax = this.x1 + aw;
				for (var xl = 0; xl < maxx; xl++) {
					var aline = this.container.line(ax, this.y1, ax, this.y2);
					aline.attr({
						"stroke": ssBase.color["SC"].htmlcolor,
						"stroke-width": 1
					});
			...
			if (rs === '3' && data.isGable && !ishybrid) {
				var aw = this.client.convert(1);
				var maxy = parseInt((this.y2 - this.y1) / aw) - 1;
				var ay = this.y1 + aw;
				for (var yl = 0; yl < maxy; yl++) {
					var aline = this.container.line(this.x1, ay, this.x2, ay);
					aline.attr({
						"stroke": ssBase.color["SC"].htmlcolor,
						"stroke-width": 1
					});
		...
		this.renderFanBeam = function () {
			...
			if (data.model["OFFSET"] != undefined) {
				lo = Number(data.model["OFFSET"].value);
				if (lo < 0) {
					lo = 1;
				}
			}
		...
		this.renderSkylight = function (alite) {
			...
			if (alite.model["OFFSETY"] != undefined) {
				to = Number(alite.model["OFFSETY"].value);
				if (to < 0) {
					lo = 1;
				}
			}
			if (alite.model["OFFSETX"] != undefined) {
				lo = Number(alite.model["OFFSETX"].value);
				if (lo < 0) {
					lo = 1;
				}
			}
