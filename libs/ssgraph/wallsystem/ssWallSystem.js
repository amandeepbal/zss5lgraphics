	...
	return function (client, data, myParent, x, y) {
		...
		this.render = function () {
			...
			for (var i = 0; i < walls.length; i++) {
				var awall = walls[i];
				if (i === 0) {
					widthf = Number(awall.model['WIDTH'].value);
					firstf = awall.model['FACING'].value;
				}

				switch (awall.model['FACING'].value) {
				case "S":
					widths += Number(awall.model['WIDTH'].value);
					break;
				case "E":
					widthe += Number(awall.model['WIDTH'].value);
					break;
				case "N":
					widthn += Number(awall.model['WIDTH'].value);
					break;
				case "W":
					widthw += Number(awall.model['WIDTH'].value);
					break;
				}
			}
			if (firstf === 'S') {
				x = widthf;
				x = this.client.convert(x);
			}
			if (widthw > widthe) {
				y += this.client.convert(widthw - widthe);
			}
		...
		this.renderPost = function (color, w, d, f) {
			...
			extrusion.svg = extrusion.container.polygon([
				...
				extrusion.x4, extrusion.y4 + nd,
				extrusion.x4 - nd, extrusion.y4 + nd,
				extrusion.x4 - nd, extrusion.y1
