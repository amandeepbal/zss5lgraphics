sap.ui.define([
	...
	return function (client, data, myParent, x, y) {
		...
		this.render = function () {
			...
			for (var i = 0; i < walls.length; i++) {
				var awall = walls[i];
				if (awall.model['FACING'].value === 'N') {
					if (Number(awall.model['WIDTH'].value) > maxwidth) {
						maxwidth += Number(awall.model['WIDTH'].value);
					}
				}
			}
			maxwidth = maxwidth / 2;
			this.laste.x1 = maxwidth;
			this.laste.x2 = maxwidth;
			this.laste.x3 = maxwidth;
			this.laste.x4 = maxwidth;
