sap.ui.define([
	...
	return function (client, data, myParent, x, y) {
		...
		this.render = function () {
			...
			var isReady = false;
			if (data.model["IS.READY"] != undefined) {
				isReady = data.model["IS.READY"].value;
			}
			if (!isReady) {
				return this;
			}
			...
			if (data.model["HAS.GABLE.POST"] != undefined) {
				this.isGable = data.model["HAS.GABLE.POST"].value;
			}
			if (!this.isGable) {
				this.renderStudio();
				return this;
		...
		this.renderHeaderTrack = function () {
			...
			this.headerextrusion.x2 = this.openings.x2;
			this.headerextrusion.y2 = this.headerextrusion.y1 + (this.lh - this.rh);
			//			this.headerextrusion.svg = this.headerextrusion.container.polygon([
			...
			this.headerextrusion.svg = this.headerextrusion.container.polygon([
				this.headerextrusion.x1, this.headerextrusion.y1,
				this.headerextrusion.x2, this.headerextrusion.y2,
				this.headerextrusion.x2, this.headerextrusion.y2 + this.tw,
				this.headerextrusion.x1, this.headerextrusion.y1 + this.tw,
				this.headerextrusion.x1, this.headerextrusion.y1
			]);
		...
		this.renderBeginPost = function () {
			...
			var y1 = this.ground;
			if (this.isGable && this.side === "R") {
				y1 = this.ground + (this.ground - this.rh);
			}
			if (this.isGable && this.side === "L") {
				y1 = this.ground + (this.ground - this.lh);
			}
		...
		this.renderReceiverFiller = function () {
			...
			if (!this.isGable) {
				if (this.openings.container != undefined) {
					x = this.openings.x2;
					x = x - w;
					var datap = core.getModel(this.getData(), "exEndPost");
					if (datap != undefined) {
						var rp = this.client.convert(Number(datap.model["WIDTH"].value));
						x = x - rp;
					}

					var datar = core.getModel(this.getData(), "modWallReceiver");
					if (datar != undefined) {
						var rw = this.client.convert(Number(datar.model["WIDTH"].value));
						x = x - rw;
					}
				}
			} else {
		...
		this.renderGableLeft = function () {
			...
			var data1 = core.getModel(this.getData(), "wsWallGableLeft");
			...
			if (lh != rh) {
				...
				this.pitch = this.trap.pitch;
			}
			if (lh > rh) {
				this.drop = w * this.trap.pitch;
			} else {
				this.rise = (w * this.trap.pitch) * -1;
			}
		...
		this.renderGableRight = function () {
			...
			var data1 = core.getModel(this.getData(), "wsWallGableRight");
			var pitch = data1.model.pitch;
