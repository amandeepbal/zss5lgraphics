sap.ui.define([
	...
	return function (client, data, myParent, x, y) {
		...
		this.renderWallBeginPost = function (wdata) {
			...
			this.renderPost(color, w, d, wdata.facing);
		};
		this.renderWallStarter = function (wdata) {
			...
			this.renderSection(color, w, d, wdata.facing);
		};
		...
		this.renderMod = function (data) {
			...
			l = this.client.convert(l);
			d = this.client.convert(d);
			this.renderSection(color, w, d, data.facing);
		};
		...
		this.renderPost = function (color, w, d, f) {
			...
			var p = "";
			if (this.prevwall != undefined) {
				p = this.lastwall.facing;
			}
			extrusion.w = w;
			extrusion.d = d;
			this.calcposition(this.lastx1, this.lasty1, extrusion, f);
		...
		this.positionFromEast = function (x, y, e, f) {
			var le = this.laste;
			var x = 0;
			var y = 0;
		...
		this.calcPositionFromNorth = function (x, y, e, f) {
			var le = this.laste;
			var x = 0;
			var y = 0;
		...
		this.calcPositionFromWest = function (x, y, e, f) {
			var lx = this.lastx;
			var ly = this.lasty;
