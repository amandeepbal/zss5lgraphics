sap.ui.define([
	...
	"zss5lgraphics/libs/ssgraph/common/ssTrapScreen"
], function(ssMath,core,ssBase,ssTrapScreen) {
	return function(myParent,x,y,w,lh,rh) {
		...
		this.render = function() {
			...
			this.paneo.y2 = this.paneo.y1 + lh;
			this.paneo.y3 = this.paneo.y2 - rh;

			this.paneo.svg = this.paneo.container.polygon([this.paneo.x1, this.paneo.y1,
            ...
            if(this.paneo.lh >= this.paneo.rh){
            	centery = this.paneo.y1 + (this.paneo.lh/2);
            	centery  = centery + 10;
            }	
            if(this.paneo.rh > this.paneo.lh){
            	centery = this.paneo.y1 + (this.paneo.rh/2);
            	centery  = centery + 10;
            }	
//			this.paneo.svgx = this.paneo.container.line(centerx+5,centery-0,centerx-15,centery+10);
//			this.paneo.svgx.attr({fill: ssBase.color["DG"].htmlcolor,"stroke": "#000", "stroke-width": 1});
			
//			this.paneo.svgx = this.paneo.container.line(centerx+20,centery-10,centerx-20,centery+30);
//			this.paneo.svgx.attr({fill: ssBase.color["SG"].htmlcolor,"stroke": "#000", "stroke-width": 1});
			
//			this.paneo.svgx = this.paneo.container.line(centerx+15,centery+5,centerx-5,centery+25);
//			this.paneo.svgx.attr({fill: ssBase.color["SG"].htmlcolor,"stroke": "#000", "stroke-width": 1});
			...
			this.isReady = true;
		    this.screen = new ssTrapScreen(this,this.paneo.x1,this.paneo.y1).render();	
