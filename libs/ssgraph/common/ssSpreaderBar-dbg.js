	...
	return function(myParent,x,y) {
		...
		this.render = function() {
            ...
            if(data.isHorizontal){
 	        var toffset = 0;
 	        var boffset = 0;
 	        if(data.model["TOP.OFFSET"] !=undefined){
               toffset = data.model['TOP.OFFSET'].value;
	           toffset = this.client.convert(toffset);
 	        }   
 	        if(data.model["BOTTOM.OFFSET"] !=undefined){
               boffset = data.model['BOTTOM.OFFSET'].value;
	           boffset = this.client.convert(boffset);
 	        }   
              ...
              if(toffset > 0){
              	this.y = oy + toffset;
              	if(this.y > (oy + (this.getParent().paneo.h / 2))){
                  this.y = oy + (this.getParent().paneo.h / 2);
              	}
              }
              else{
                if(boffset > 0){
              	  this.y = oy + (this.getParent().paneo.h) - boffset ;
              	  if(this.y < (oy + (this.getParent().paneo.h / 2))){
                    this.y = oy + (this.getParent().paneo.h / 2);
              	  }
                }
