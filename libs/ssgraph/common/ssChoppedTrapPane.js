sap.ui.define([
	...
	"zss5lgraphics/libs/ssgraph/common/ssChoppedTrapScreen"
], function(ssMath,core,ssBase,ssChoppedTrapScreen) {
	return function(myParent,x,y,w,lh,rh,tl) {
		...
		this.render = function() {
			...
			if (lh > rh) {
				this.paneo.x3 = this.paneo.x1 + tl;
				this.paneo.svg = this.paneo.container.polygon([
					this.paneo.x3, this.paneo.y1,
				...
				this.paneo.x3 = this.paneo.x2 - tl;
				this.paneo.svg = this.paneo.container.polygon([
					this.paneo.x1, this.paneo.y1,
					this.paneo.x1, this.paneo.y2,
					this.paneo.x2, this.paneo.y2,
					this.paneo.x2, this.paneo.y3,
					this.paneo.x3, this.paneo.y3,
				]);
			}
            ...
            if(this.paneo.lh >= this.paneo.rh){
            	centery = this.paneo.y3 + (this.paneo.lh/2);
//            	centery  = centery + 10;
            	centery  = centery + 0;
            }	
            if(this.paneo.rh > this.paneo.lh){
            	centery = this.paneo.y3 + (this.paneo.rh/2);
//            	centery  = centery + 10;
            	centery  = centery + 0;
            }	
            ...
            if(ttype === 'G'){
			  ...
			  this.paneo.svgx.attr({fill: ssBase.color["SG"].htmlcolor,"stroke": "#000", "stroke-width": 1});
      	         ...
      	         if (lh >= rh) { centery = this.paneo.y1 + (this.paneo.lh/2);}
      	         if (rh > lh) { centery = this.paneo.y3 + (this.paneo.rh/2);}
			...
			this.isReady = true;
		    this.screen = new ssChoppedTrapScreen(this,this.paneo.x1,this.paneo.y1).render();	
