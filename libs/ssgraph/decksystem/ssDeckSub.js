	...
	return function(client, data, myParent, x, y) {
		...
		this.renderJoists = function() {
			...
			if (data.model["DECK.DIRECTION"] != undefined) {
				direction = data.model["DECK.DIRECTION"].value;
			}
			if (data.model["LENGTH"] != undefined) {
			...
			if (data.model["DECK.JOIST.SPACING"] != undefined) {
				s = data.model["DECK.JOIST.SPACING"].value;
			}
			...
			var objmods = core.getModels(data, 'dsJoist');
		...
		this.renderBeams = function() {
			...
			for (var i = 0; i < objmods.length; i++) {
				var amod = objmods[i];
				if (amod.Code != 'dsCChannel' && amod.Code != 'dsBoxBeam') {
					continue;
				}
		...
		this.renderSideW = function(data, x, y) {
			...
			this.sideW.svg = this.sideE.container.polygon([this.sideW.x1, this.sideW.y1,
		...
		this.renderPlanks = function() {
			var d = '1';

			if (data.model["DECK.DIRECTION"] != undefined) {
				d = data.model["DECK.DIRECTION"].value;
			}
			var objDeckPlanks = core.getModel(this.getData().model.parent, 'dsDeckPlanks');
			if (objDeckPlanks === undefined) {
				return;
			}

			var objmods = core.getModels(objDeckPlanks, 'dsPlank');
			if (objmods === undefined) {
				return;
			}
			objmods.sort(function(a, b) {
